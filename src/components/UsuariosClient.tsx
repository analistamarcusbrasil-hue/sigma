...
export function UsuariosClient({ usuarios }: { usuarios: PerfilSigma[] }) {
  ...
  useEffect(() => { listarObreiros().then(setObreiros).catch(() => { setMensagemErro(true); setMensagem("Não foi possível carregar os obreiros."); }); }, []);
  ...
  async function salvar(event: React.FormEvent) {
    ...
    try {
      if (editando) await atualizarUsuario({ id: editando.id, nome: form.nome.trim(), perfil: form.perfil, obreiroId: form.obreiroId || null, permissoes: form.permissoes });
      else await convidarUsuario({ nome: form.nome.trim(), email: form.email.trim(), perfil: form.perfil, obreiroId: form.obreiroId || null, permissoes: form.permissoes });
