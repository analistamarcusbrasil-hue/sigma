...
export type StatusPrestacaoFinal="Em elaboração"|"Em conferência"|"Pronta para aprovação"|"Aprovada"|"Bloqueada";
export type PrestacaoFinal={id:string;gestaoId:string;status:StatusPrestacaoFinal;saldoInicial:number;totalReceitas:number;totalDespesas:number;saldoFinal:number;mensalidadesRecebidas:number;mensalidadesPendentes:number;totalTronco:number;despesasPendentes:number;semComprovante:number;caixaFisicoFinal:number;contaBancariaFinal:number;creditosReceber:number;obrigacoesPagar:number;saldoLiquidoRepasse:number;observacoesTesoureiro:string;observacoesVeneravel:string;pendenciasAdministrativas:string;justificativaFechamentos:string;observacoesComprovantes:string;observacoesGerais:string;aprovadoPor:string;aprovadoEm:string;criadoEm:string;atualizadoEm:string};
...
export async function listarPrestacoesFinais():Promise<PrestacaoFinal[]>{const loja=await obterLojaAtual();const{data,error}=await createClient().from("prestacoes_finais").select("*").eq("loja_id",loja.id).order("criado_em",{ascending:false});if(error)throw new Error(error.message);return(data??[]).map(i=>({id:i.id,gestaoId:i.administracao_id,status:i.status as StatusPrestacaoFinal,saldoInicial:Number(i.saldo_inicial),totalReceitas:Number(i.total_receitas),totalDespesas:Number(i.total_despesas),saldoFinal:Number(i.saldo_final),mensalidadesRecebidas:Number(i.mensalidades_recebidas),mensalidadesPendentes:Number(i.mensalidades_pendentes),totalTronco:Number(i.total_tronco),despesasPendentes:Number(i.despesas_pendentes),semComprovante:Number(i.lancamentos_sem_comprovante),caixaFisicoFinal:Number(i.caixa_fisico_final),contaBancariaFinal:Number(i.conta_bancaria_final),creditosReceber:Number(i.creditos_a_receber),obrigacoesPagar:Number(i.obrigacoes_a_pagar),saldoLiquidoRepasse:Number(i.saldo_liquido_repasse),observacoesTesoureiro:i.observacoes_tesoureiro??"",observacoesVeneravel:i.observacoes_veneravel??"",pendenciasAdministrativas:i.pendencias_administrativas??"",justificativaFechamentos:i.justificativa_fechamentos??"",observacoesComprovantes:i.observacoes_comprovantes??"",observacoesGerais:i.observacoes_gerais??"",aprovadoPor:i.aprovado_por??"",aprovadoEm:i.aprovado_em??"",criadoEm:i.criado_em,atualizadoEm:i.atualizado_em}));}
export async function salvarPrestacaoFinal(i:PrestacaoFinal):Promise<PrestacaoFinal>{const loja=await obterLojaAtual();const p={loja_id:loja.id,administracao_id:i.gestaoId,status:i.status,saldo_inicial:i.saldoInicial,total_receitas:i.totalReceitas,total_despesas:i.totalDespesas,saldo_final:i.saldoFinal,mensalidades_recebidas:i.mensalidadesRecebidas,mensalidades_pendentes:i.mensalidadesPendentes,total_tronco:i.totalTronco,despesas_pendentes:i.despesasPendentes,lancamentos_sem_comprovante:i.semComprovante,caixa_fisico_final:i.caixaFisicoFinal,conta_bancaria_final:i.contaBancariaFinal,creditos_a_receber:i.creditosReceber,obrigacoes_a_pagar:i.obrigacoesPagar,saldo_liquido_repasse:i.saldoLiquidoRepasse,observacoes_tesoureiro:i.observacoesTesoureiro.trim()||null,observacoes_veneravel:i.observacoesVeneravel.trim()||null,pendencias_administrativas:i.pendenciasAdministrativas.trim()||null,justificativa_fechamentos:i.justificativaFechamentos.trim()||null,observacoes_comprovantes:i.observacoesComprovantes.trim()||null,observacoes_gerais:i.observacoesGerais.trim()||null};const q=i.id?createClient().from("prestacoes_finais").update(p).eq("id",i.id).eq("loja_id",loja.id):createClient().from("prestacoes_finais").insert(p);const{data,error}=await q.select("id,aprovado_por,aprovado_em,criado_em,atualizado_em").single();if(error)throw new Error(error.message);return{...i,id:data.id,aprovadoPor:data.aprovado_por??"",aprovadoEm:data.aprovado_em??"",criadoEm:data.criado_em,atualizadoEm:data.atualizado_em};}
...
export type RepasseGestao={id:string;gestaoOrigemId:string;prestacaoFinalId:string;status:string;dataRepasse:string;caixa:number;banco:number;creditos:number;obrigacoes:number;saldoLiquido:number;pendenciasFinanceiras:string;pendenciasAdministrativas:string;documentosPendentes:string;observacoes:string;justificativa:string;responsavelRepasseId:string;responsavelRecebimentoId:string;finalizadoEm:string};
export async function listarRepasses():Promise<RepasseGestao[]>{const loja=await obterLojaAtual();const{data,error}=await createClient().from("repasses_gestao").select("*").eq("loja_id",loja.id);if(error)throw new Error(error.message);return(data??[]).map(i=>({id:i.id,gestaoOrigemId:i.gestao_origem_id,prestacaoFinalId:i.prestacao_final_id??"",status:i.status,dataRepasse:i.data_repasse??"",caixa:Number(i.caixa),banco:Number(i.banco),creditos:Number(i.creditos),obrigacoes:Number(i.obrigacoes),saldoLiquido:Number(i.saldo_liquido),pendenciasFinanceiras:i.pendencias_financeiras??"",pendenciasAdministrativas:i.pendencias_administrativas??"",documentosPendentes:i.documentos_pendentes??"",observacoes:i.observacoes??"",justificativa:i.justificativa_divergencia??"",responsavelRepasseId:i.responsavel_repasse_id??"",responsavelRecebimentoId:i.responsavel_recebimento_id??"",finalizadoEm:i.finalizado_em??""}));}
export async function salvarRepasse(i:RepasseGestao){const loja=await obterLojaAtual();const p={loja_id:loja.id,gestao_origem_id:i.gestaoOrigemId,prestacao_final_id:i.prestacaoFinalId||null,status:i.status,data_repasse:i.dataRepasse||null,caixa:i.caixa,banco:i.banco,creditos:i.creditos,obrigacoes:i.obrigacoes,pendencias_financeiras:i.pendenciasFinanceiras||null,pendencias_administrativas:i.pendenciasAdministrativas||null,documentos_pendentes:i.documentosPendentes||null,observacoes:i.observacoes||null,justificativa_divergencia:i.justificativa||null,responsavel_repasse_id:i.responsavelRepasseId||null,responsavel_recebimento_id:i.responsavelRecebimentoId||null};const q=i.id?createClient().from("repasses_gestao").update(p).eq("id",i.id):createClient().from("repasses_gestao").insert(p);const{data,error}=await q.select("id,saldo_liquido,finalizado_em").single();if(error)throw new Error(error.message);return{...i,id:data.id,saldoLiquido:Number(data.saldo_liquido),finalizadoEm:data.finalizado_em??""};}
